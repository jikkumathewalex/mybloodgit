...
function searchDistrict() {
  ...
  <tbody>
		     <?php
              $query=mysqli_query($con,"SELECT REG.*,LOG.*, HOS.*, DIS.*, ACC.* FROM tbl_reg as REG, tbl_login as LOG, tbl_hospital as HOS, tbl_district as DIS, tbl_accreditation as ACC WHERE LOG.login_id=REG.login_id AND HOS.reg_id=REG.reg_id AND REG.district_id=DIS.district_id AND HOS.status!=2 ORDER BY REG.reg_date DESC");
              ...
              while($row=mysqli_fetch_array($query))
              {
               echo '<tr>
                              ...
                              <td>'.$row['accreditation'].'</td>
		                          <td><a href="../../../hos_acc_certificates/'.$row['accreditation_certificate'].'" target="_blank">'.$row['accreditation_certificate'].'</a></td>
		                          <td>';

		           $query1=mysqli_query($con,'SELECT * FROM  tbl_login where login_id="'.$row['login_id'].'" AND status=1');
               if(mysqli_num_rows($query1))
               {?>
                <form action="admin_hospital_cancel_approval_action.php" method="post">
                ...
                {?>
                  <form action="admin_hospital_approval_action.php" method="post">
               ...
               <td>
               <form action="admin_hospital_delete_action.php" method="post">
