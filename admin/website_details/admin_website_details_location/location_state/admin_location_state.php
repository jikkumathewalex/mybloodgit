...
<br>
      ...
      <form class="container" action="admin_add_location_state_action.php" method="post">
      <center>     
      
      
      <div class="input-group mb-3">
          <div class="input-group-prepend">
              <span class="input-group-text" id="basic-addon1" style="background-color:#bb0a1e;color:white;width:220px">State</span>
          </div>
               <input type="text" class="form-control" id="txtState" name="txtState" oninput="this.reportValidity()" required="" aria-describedby="emailHelp" title="Enter State" placeholder="Enter State">&nbsp
           </div>
...
<br>
  ...
  <tbody>
     <?php
          $query=mysqli_query($con,"SELECT * FROM tbl_state where status=1 ORDER BY state ASC");
          while($row=mysqli_fetch_array($query))
          {
           ...
           $query1=mysqli_query($con,'SELECT * FROM  tbl_state where state_id="'.$row['state_id'].'" AND status=1');
           if(mysqli_num_rows($query1))
           {?>
            <form action="admin_cancel_location_state_action.php" method="post">
              <input type="hidden" name="delete" id="delete" value="<?php echo $row['state_id'] ?>">
