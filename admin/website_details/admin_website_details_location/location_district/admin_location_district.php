          ...
          <div class="float-right">
            ...
            <i class="fas fa-envelope-square"></i> <?php echo $row['email']?>&nbsp
                ...
                <center>
                  ...
                  <a class="dropdown-item" href="../location_city/admin_location_city.php">City</a>
       </center>
...
<br>
      ...
      <form class="container" action="admin_add_location_district_action.php" method="post">&nbsp
        
        
          <div class="input-group mb-3">
          <div class="input-group-prepend">
          <select name="ddstate" id="ddstate" style="background-color:#bb0a1e" title="Select State" class="btn btn-secondary dropdown-toggle" required>
          ...
          <?php
             ...
             while($row=mysqli_fetch_array($query))
               {
                echo "<option style='background-color:white;color:black' value='".$row['state']."'>".$row['state']."</option>";
               }?>
          </select>&nbsp;&nbsp;
              <span class="input-group-text" id="basic-addon1" style="background-color:#bb0a1e;color:white;width:220px">District</span>
          </div>
               <input type="text" class="form-control" id="txtDistrict" name="txtDistrict" oninput="this.reportValidity()" required="" aria-describedby="emailHelp" title="Enter District" placeholder="Enter District">&nbsp
           </div>
           <button type="submit" name="submit" class="btn btn-primary float-right" style="background:#bb0a1e;border:#bb0a1e">Submit</button>
        </form>
<br><br><br><br><br>
<center>
...
function searchDistrict() {
  ...
  <tbody>
     <?php
          $query=mysqli_query($con,"SELECT ST.*,DIS.* FROM tbl_state ST,tbl_district DIS WHERE ST.state_id=DIS.state_id ORDER BY state");
          while($row=mysqli_fetch_array($query))
          {
           echo '<tr>
                          <td>'.$row['state'].'</td>
           ...
           $query1=mysqli_query($con,'SELECT * FROM  tbl_district where district_id="'.$row['district_id'].'" AND status=1');
           if(mysqli_num_rows($query1))
           {?>
            <form action="admin_cancel_location_district_action.php" method="post">
              <input type="hidden" name="delete" id="delete" value="<?php echo $row['district_id'] ?>">
