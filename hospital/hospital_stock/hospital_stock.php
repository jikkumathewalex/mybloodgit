...
document.getElementById("sd").setAttribute("max", today);
     ...
     {
      var val = document.getElementById('txtHStock').value;

      if (!val.match(/^(A|B|AB|O)[+-]$/)) 
      {
        alert('Please Enter Valid Blood Group');
	   document.getElementById('txtHStock').value = "";
        return false;
    }
...
<nav class="navbar navbar-light bg-light">
      ...
      <nav class="navbar" style="background-color:#bb0a1e;color:white">
          ...
          <form action="../hospital_request/hospital_request.php"><button type="submit" class="btn btn-outline-danger" style="color:white">Notification</button></form>
          ...
          <div class="float-right">
            ...
            <i class="fas fa-envelope-square"></i> <?php echo $row['email']?>&nbsp
              ...
              <center>
                <a class="dropdown-item" href="../hospital_request/hospital_request.php">Notification</a>
                <a class="dropdown-item" href="../feedback/hospital_feedback.php">Feedback</a>
              </center>

                <div class="dropdown-divider"></div>
                <center>
                <a class="dropdown-item" href="../edit_profile/hosp_edit_profile.php">Edit Profile</a>
      ...
      <div style="color:#bb0a1e" class="card-header" >
          <center><h2>HOSPITAL STOCK</h2></center><br>
      </div>
      <br>
      <form class="container form-outline" action="hospital_stock_add_action.php" method="post">
...
&nbsp;&nbsp;
    ...
    <tbody>
         <?php
              $query=mysqli_query($con,"SELECT HOS.*, BG.* FROM tbl_hos_stock HOS, tbl_blood_group BG WHERE HOS.status=1 AND HOS.available_blood_group=BG.blood_group_id");
              if(!mysqli_num_rows($query))
               echo "<center>Oops, no entry</center>";
              while($row=mysqli_fetch_array($query))
              {
               echo '<tr>
                              <td>'.$row['packetid'].'</td>
               ...
               $query1=mysqli_query($con,'SELECT * FROM  tbl_hos_stock where hos_stock_id="'.$row['hos_stock_id'].'" AND status=1');
               if(mysqli_num_rows($query1))
               {?>
                <form action="hospital_stock_cancel_action.php" method="post">
                  <input type="hidden" name="delete" id="delete" value="<?php echo $row['hos_stock_id'] ?>">
